...
proc adi_project_run {project_name} {
  ...
  wait_on_run synth_1
  open_run synth_1
  report_timing_summary -file ${actual_project_name}_timing_synth.log
  ...
  wait_on_run impl_1
  open_run impl_1
  report_timing_summary -warn_on_violation -file ${actual_project_name}_timing_impl.log

  if {[info exists ::env(ADI_GENERATE_UTILIZATION)]} {
    set csv_file ${actual_project_name}_resource_utilization.csv
    if {[ catch {
  ...
  if {[info exists ::env(ADI_GENERATE_XPA)]} {
    set csv_file ${actual_project_name}_power_analysis.csv
    set Layers "8to11"
    set CapLoad "20"
  ...
  if {[regexp { (\d+) register} $timing_check -> num_regs]} {
    ...
    if {[info exist num_regs]} {
      if {$num_regs > 0} {
        ...
        check_timing -override_defaults no_clock -verbose -file ${actual_project_name}_no_clock.log
      }
    }
